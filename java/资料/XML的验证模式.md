...
XML Schema语言（XML Schemas Definition）描述了XML的结构。

#### 注意

在使用XML Schema文档对XML实例文档就行检验时，需要声明**名称空间**（xmlns=http://www.Springframework.org/schema/beans）,还需要指定名称空间对应的XML Schema文档的**存储位置**（xsi:schemaLocation="http://www.springframework.org/schema/beans/Spring-beans.xsd"）
